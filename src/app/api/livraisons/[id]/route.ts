...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const [livraison] = await sql`
      SELECT l.*, c.total AS commande_total, c.adresse_livraison,
        CASE WHEN cl.type_client = 'entreprise' THEN cl.titre ELSE CONCAT(cl.prenom, ' ', cl.nom) END AS client_nom,
        cl.telephone AS client_telephone, cl.email AS client_email, cl.adresse AS client_adresse,
        u.nom AS livreur_nom, bl.numero_bon AS numero_bon_livraison, bl.id AS bon_livraison_id, bl.date_livraison_prevue
      FROM livraisons l
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const [avant] = await sql`
      SELECT l.commande_id, l.livreur_id,
        CASE WHEN cl.type_client = 'entreprise' THEN cl.titre ELSE CONCAT(cl.prenom, ' ', cl.nom) END AS client_nom,
    ...
    if (statut === 'livree' && avant) {
      await sql`UPDATE commandes SET statut = 'livree', updated_at = NOW() WHERE id = ${livraison.commande_id}`;
      await onLivraisonTerminee(Number(id), livraison.commande_id, avant.client_nom, avant.client_user_id ? Number(avant.client_user_id) : undefined);
      await onStatutCommande(livraison.commande_id, 'pret_livraison', 'livree', avant.client_nom, avant.client_user_id ? Number(avant.client_user_id) : undefined, avant.livreur_id ? Number(avant.livreur_id) : undefined);
    }
