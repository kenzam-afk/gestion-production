...
>>>>>>> c1e627906dc56c70dde9c0c4a3de57055d542bb4
...
<<<<<<< HEAD
  const [form, setForm] = useState({
    ...
    nif: '', titre: '', annee_creation: '', siege_social: '',
...
<<<<<<< HEAD
    const res = await fetch('/api/clients', {
      ...
      body: JSON.stringify({ type_client: typeClient, ...form }),
    });
    if (res.ok) {
      setForm({ email: '', telephone: '', adresse: '', nif: '', titre: '', annee_creation: '', siege_social: '', nom: '', prenom: '', date_naissance: '', nin: '' });
      setShowForm(false);
      fetchClients();
    }
  };
      ...
      {showForm && (
        <div className="bg-white border rounded-xl p-6 mb-6 shadow-sm">
          ...
          <div className="flex gap-4 mb-6">
            <button
              onClick={() => setTypeClient('individuel')}
              className={`flex-1 py-3 rounded-lg border-2 font-medium transition-all ${
                typeClient === 'individuel' ? 'border-blue-600 bg-blue-50 text-blue-600' : 'border-gray-200 text-gray-500'
              }`}
            >
              👤 Client Individuel
            </button>
            <button
              ...
              className={`flex-1 py-3 rounded-lg border-2 font-medium transition-all ${
                typeClient === 'entreprise' ? 'border-blue-600 bg-blue-50 text-blue-600' : 'border-gray-200 text-gray-500'
              }`}
            >
              🏢 Entreprise
            </button>
          ...
          <div className="grid grid-cols-2 gap-4">
              ...
              <>
                <div>
                  ...
                  <input type="text" value={form.nom} onChange={e => setForm({ ...form, nom: e.target.value })}
                    className="w-full border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Ex: Benali" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Prénom *</label>
                  <input type="text" value={form.prenom} onChange={e => setForm({ ...form, prenom: e.target.value })}
                    className="w-full border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Ex: Karim" />
                </div>
                <div>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">NIN (Numéro d'identité national)</label>
                  <input type="text" value={form.nin} onChange={e => setForm({ ...form, nin: e.target.value })}
                    className="w-full border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Ex: 12345678901234567890" />
                </div>
              </>
            )}
            ...
            {typeClient === 'entreprise' && (
              <>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Titre / Raison sociale *</label>
                  <input type="text" value={form.titre} onChange={e => setForm({ ...form, titre: e.target.value })}
                    className="w-full border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Ex: SARL Bati-Construct" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">NIF *</label>
                  <input type="text" value={form.nif} onChange={e => setForm({ ...form, nif: e.target.value })}
                    className="w-full border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Ex: 123456789" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Année de création</label>
                  <input type="number" value={form.annee_creation} onChange={e => setForm({ ...form, annee_creation: e.target.value })}
                    className="w-full border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Ex: 2010" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Siège social</label>
                  <input type="text" value={form.siege_social} onChange={e => setForm({ ...form, siege_social: e.target.value })}
                    className="w-full border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Ex: Alger Centre" />
                </div>
              </>
            )}
            ...
            <div>
              ...
              <input type="email" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })}
                className="w-full border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="contact@example.com" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Téléphone</label>
              <input type="text" value={form.telephone} onChange={e => setForm({ ...form, telephone: e.target.value })}
                className="w-full border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="0555 123 456" />
            </div>
            <div className="col-span-2">
              <label className="block text-sm font-medium text-gray-700 mb-1">Adresse</label>
              <input type="text" value={form.adresse} onChange={e => setForm({ ...form, adresse: e.target.value })}
                className="w-full border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="12 Rue Didouche Mourad, Alger" />
            </div>
          </div>

          <div className="flex gap-3 mt-4">
            <button onClick={handleSubmit} className="bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700">Enregistrer</button>
            <button onClick={() => setShowForm(false)} className="bg-gray-200 text-gray-700 px-6 py-2 rounded-lg hover:bg-gray-300">Annuler</button>
=======
          ...
          <table className="w-full">
            <thead className="bg-gray-50 border-b">
              <tr>
                ...
                <th className="text-left px-4 py-3 text-sm font-semibold text-gray-600">Nom / Titre</th>
                <th className="text-left px-4 py-3 text-sm font-semibold text-gray-600">NIN / NIF</th>
                <th className="text-left px-4 py-3 text-sm font-semibold text-gray-600">Date naissance</th>
            ...
            <tbody>
              {clients.length === 0 ? (
                <tr><td colSpan={6} className="text-center py-8 text-gray-400">Aucun client enregistré</td></tr>
              ) : (
                clients.map(c => (
                  <tr key={c.id} className="border-b hover:bg-gray-50">
                    <td className="px-4 py-3">
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                        c.type_client === 'entreprise' ? 'bg-purple-100 text-purple-700' : 'bg-green-100 text-green-700'
                      }`}>
