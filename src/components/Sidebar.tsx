...
function AlertBell() {
  ...
  useEffect(() => {
    fetchAlertes();
    const t = setInterval(fetchAlertes, 60000);
  ...
  return (
    <div ref={ref} style={{ position: 'relative', flexShrink: 0 }}>
      ...
      <button className="ab-btn" onClick={() => setOpen(o => !o)}>
        <AlertTriangle size={14} color={nb > 0 ? '#ef4444' : 'rgba(255,255,255,0.5)'} />
        ...
        <div className="ab-dropdown">
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'12px 14px', borderBottom:'1px solid rgba(239,68,68,.15)', background:'rgba(239,68,68,.06)' }}>
            <div style={{ display:'flex', alignItems:'center', gap:7 }}>
              ...
              <span style={{ fontSize:13, fontWeight:700, color:'#fca5a5' }}>Alertes système</span>
              {nb > 0 && <span style={{ background:'#ef4444', color:'white', fontSize:10, fontWeight:700, padding:'1px 7px', borderRadius:20 }}>{nb}</span>}
            </div>
          ...
          <div style={{ maxHeight:380, overflowY:'auto' }}>
            {alertes.length === 0 ? (
              ...
              <Link key={i} href={a.lien} onClick={() => setOpen(false)} style={{ textDecoration:'none', display:'block' }}>
                <div className="ab-item">
                  <div style={{ width:32, height:32, borderRadius:9, flexShrink:0, background:a.niveau==='danger'?'rgba(239,68,68,.15)':'rgba(245,158,11,.15)', border:`1px solid ${a.niveau==='danger'?'rgba(239,68,68,.3)':'rgba(245,158,11,.3)'}`, display:'flex', alignItems:'center', justifyContent:'center' }}>
                    <AlertTriangle size={14} color={a.niveau==='danger'?'#ef4444':'#f59e0b'} />
                  </div>
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ fontSize:12.5, fontWeight:600, color:a.niveau==='danger'?'#fca5a5':'#fcd34d', lineHeight:1.3 }}>{a.titre}</div>
                    <div style={{ fontSize:11.5, color:'#5c5a7a', marginTop:2, lineHeight:1.4 }}>{a.detail}</div>
                  </div>
                  <div style={{ width:8, height:8, borderRadius:'50%', flexShrink:0, marginTop:4, background:a.niveau==='danger'?'#ef4444':'#f59e0b', boxShadow:`0 0 6px ${a.niveau==='danger'?'#ef4444':'#f59e0b'}` }} />
                </div>
              </Link>
            ))}
          </div>
    ...
    <>
      <style>{`
        ...
        .sidebar-inner {
          ...
          padding: 0 16px; height: 56px; gap: 4px;
        ...
        .logo-name { font-weight: 800; font-size: 13px; color: #f1f0ff; letter-spacing: -0.02em; line-height: 1; }
        .logo-sub  { font-size: 8px; color: #7c3aed; font-weight: 500; text-transform: uppercase; letter-spacing: 0.1em; margin-top: 2px; }
        .divider-v { width: 1px; height: 24px; background: rgba(255,255,255,0.08); flex-shrink: 0; margin: 0 2px; }
        .nav-scroll {
          display: flex; align-items: center; gap: 1px;
          overflow-x: auto; flex: 1; scrollbar-width: none; padding: 2px 0;
          min-width: 0;
        }
        ...
        .nav-link {
          ...
          font-size: 11.5px; font-weight: 500; color: rgba(160,157,192,0.8);
        ...
        .nav-icon-wrap { width: 18px; height: 18px; display: flex; align-items: center; justify-content: center; border-radius: 5px; flex-shrink: 0; }
        .right-section {
        ...
        .avatar {
          ...
          font-family: 'Outfit', sans-serif; box-shadow: 0 0 8px rgba(124,58,237,0.3);
          flex-shrink: 0;
        }
        .profile-name { font-size: 11px; font-weight: 600; color: #e2e0ff; line-height: 1.1; }
        .profile-role { font-size: 8px; color: #7c3aed; font-weight: 600; text-transform: uppercase; letter-spacing: 0.06em; }
        .logout-btn {
          display: flex; align-items: center; gap: 5px;
          padding: 6px 10px; border-radius: 8px;
          border: 1px solid rgba(255,255,255,0.07);
          background: transparent; cursor: pointer;
          font-size: 11.5px; font-weight: 500; color: rgba(160,157,192,0.7);
          ...
          <div className="sidebar-inner">
            ...
            <Link href="/admin" className="logo-wrap">
              <div className="logo-icon">
                <Package size={15} color="white" style={{ position:'relative', zIndex:1 }} />
              </div>
              <div>
                ...
                return (
                  <Link key={item.name} href={item.href} className={`nav-link${isActive ? ' active' : ''}`}>
                    <div className="nav-icon-wrap" style={{ background: isActive ? item.color + '22' : 'transparent' }}>
                      <Icon size={13} color={isActive ? item.color : 'currentColor'} />
                    </div>
                    <span>{item.name}</span>
                  </Link>
                );
            ...
            {/* Droite : alertes + notifs + profil + logout */}
